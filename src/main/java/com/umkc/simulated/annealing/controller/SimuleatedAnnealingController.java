...
public class SimuleatedAnnealingController {
	...
	public ResponseEntity<String> uploadSingleFileExample2(@RequestParam MultipartFile file,
			@RequestParam(required = false) String delimeter, @RequestParam double maxWeight) {
		LOGGER.info("Request contains, File: " + file.getOriginalFilename());

		if (delimeter == null || delimeter.isEmpty()) {
			delimeter = "\t";
		}
		return ResponseEntity.ok(simulatedAnnealingService.process(file, delimeter, maxWeight));
	}
